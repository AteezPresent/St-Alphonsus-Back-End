...
<script>
   ...
   {
let x = document.forms["myform"]["tid"].value;
let y = document.forms["myform"]["tname"].value;

if (x==""){
   alert("tid must be filled out");
   return false;
}

if (y==""){
   alert("tname must be filled out");
   return false;
}
   }
   </script>
<form name="myform" method="post" action="AddTeacher.php"onsubmit="return validateForm()">
            <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
            <i class=""></i><label<label for="tid">Teacher ID:</label>
         <input type="text" name="tid" id="tid" placeholder="tid">
           <br>
           <br>
           <i class=""></i><label<label for="tname">Teacher Name </label>
            <input  type="text" name="tname" id="tname" placeholder="tname">
           <br>
           <br>
           <i class=""></i><label<label for="taddress">Teacher Address</label>
            <input  type="text" name="taddress" id="taddress" placeholder="taddress">
            <br>
            <br>
            <i class=""></i><label<label for="tmobile">Teacher Mobile</label>
            <input  type="text" name="tmobile" id="tmobile" placeholder="tmobile">
            <br>
            <br>
            <i class=""></i><label<label for="tannualleave">Teacher Annual Leave</label>
            <input  type="text" name="tannualleave" id="tannualleave" placeholder="annualleave">
            <br>
            <br>
            <label>Select Wage ID:</label> 
            <select name ="wid">
               <?php
               $sql = mysqli_query($link,"SELECT wid, wamount FROM wage");
               while ($row = $sql->fetch_assoc()){
                  echo "<option value='{$row['wid']}'>
                                       {$row['wamount']}
                                        </option>";
                                     } 
          ?>
...
if (isset($_POST['submit'])){
    ...
    $tannualleave = $_POST['tannualleave'];
    $wid = $_POST['wid'];
}
$sql="INSERT INTO teacher (tid,tname,taddress,tmobile,tannualleave,wid)  VALUES ('$tid','$tname','$taddress','$tmobile','$tannualleave','$wid')";
